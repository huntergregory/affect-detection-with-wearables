...
full_coefficient <- data.frame("Terms" = c(rounded$term),
"Estimate" = c(rounded$estimate), "CI" = c("(-1.055, -2.322)", "(0.381,1.218)", "(-7.952,-5.136)", "(-2.893,-1.772)", "(0.293, 1.816)","(0.741,2.115)","(-1.519,-0.241)","(0.265,1.178)", "(-6.994, -4.478)", "(0.002,1.700)"),"MultFac" = c(MultFac), stresslikelihood)
...
\newline
Our model shows that when holding all other features at constant, the increase in one standard deviation of volume of air inhale per minute (RespiBan), the average accelerometry (both from RespiBan and E4),the average body temperature (E4), or the average heart rate variability (E4) results in the more likelihood of being stressed. On the other hand, when holding all other features at constant, the increase in one standard deviation of Number of skin conductivity responses (both from RespiBan and E4), the average accelerometry (RespiBan), the average accelerometry (E4),the average body temperature (RespiBan), or the average heart rate variability (RespiBan) results in the less likelihood of being stressed. 
\newline
One interesting finding is that an increase in chest_temp_mean leads to the less likelihood of being stressed, while an increase in wrist_TEMP_mean leads to the more likelihood of being stressed. These opposite reactions are not due to correlation because the two features have a -0.02 correlation. However, we found an increase in HRV from RespiBan results in less likelihood of being stressed, while an increase HRV from E4 leads to the more likelihood of being stressed. These may be due to the small correlation of 0.22 between the two features.
