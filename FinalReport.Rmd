...
Stress response acts physiological, behavioral, and affective forms $[12]$. Wearable devices attempt to detect stress through observable physiological data. However, because stress is highly subjective and presents differently in individuals, stress reactions are difficult to directly discern through observable physiological responses. This lack of a standard definition of stress presents as a major obstacle in the effective computing of stress responses with wearable devices $[11]$. 
...
All features were inspired by Schmidt et al. For body temperature (TEMP), we calculate mean, standard deviation, minimum, maximum, and slope from window start to finish over the raw signal. For the 3-axis accelerometry (ACC), we calculate mean, standard deviation, and the absolute integral over time for all dimensions and for the magnitude of the raw signal. For the other signals, we clean and then extract features using NeuroKit2. For Respiration (RESP), we calculated the mean and standard deviation for inhalation and exhalation duration, the inhalation/exhalation ratio, volume (amount of air taken in), and breath rate (breaths per minute). 

For electrodermal activity (EDA), we calculated the mean, standard deviation, minimum, maximum, slope, and features for the skin conductance level (SCL) and skin conductance response (SCR) components of the signal. EDA reflects the skin’s response bursts and recovery to impetus. As described in Schmidt et al, the “SCL represents a slowly varying baseline conductivity, while the SCR is a short term response to a stimulus.” We calculated correlation of SCL over time as well as the mean and standard deviation of SCL and SCR. We also calculated the number of segments, sum of startle magnitudes and startle response durations as well as the area under SCRs. See Healey et al. for a deeper description $[6]$. Additionally, we performed linear interpolation to upsample the EDA wrist data from 4 Hz to 8 Hz due to a signal processing frequency requirement in Neurokit2.

We also convert blood volume pulse (BVP) and electrocardiogram (ECG) signals to Heart Rate (HR) and Heart Rate Variability using Neurokit2. We then calculate mean and standard deviation of HR and HRV, the percent of large intervals (RR intervals differing more than 50 ms), triangular interpolation index, root mean square of RR intervals, and ultra low (0.01-0.04 Hz), low (0.04-0.15 Hz), high (0.15-0.4 Hz), and ultra high (0.4-1.0 Hz) frequency bands’ power spectral density.Finally, we had to forgo electromyogram (EMG) data since we couldn’t resolve Neurokit2 processing errors with this signal. 
...
First, we discarded frequency-based HR/HRV features since NeuroKit2 had problems distinguishing ultra low, low, high, and ultra high frequency bands. We also immediately removed standard deviations, maxima, and minima since all were highly correlated with means. We investigated correlations between chest and wrist data as a whole, but we did not find any concerning correlations.  

For 3-Axis Accerometry (ACC), out of all dimensions, we included only the mean magnitude since the magnitude is an informative summary and generalizes better when comparing wrist and chest data. There doesn’t seem to be any distinction between the two states in accelerometry. For body temperature (TEMP), we keep slope and mean, as they seem to be higher for stressed states, and the correlation between the two is 0.01. 

For Respiration (RESP), we include breath rate and volume. There seems to be an increase in breath rate for amusement condition overall, and a decrease in volume. Inhale duration and exhale duration were clearly highly correlated with volume, so we omitted those.

For electrodermal activity (EDA), number of responses/segments is much higher when amused, so we include this and forego its very correlated counterparts (sum of startle magnitudes, sum of startle response duration, area under SCRs). We also included slope to provide information about the trend in EDA, and forego its highly correlated counterpart (SCL correlation with time). Finally, we didn’t include EDA/SCL/SCR mean, since these are not as informative as the trend (via slope) and startle characteristics.
...
Note : More boxplots for other features can be found in the Appendix. 
```{r figures, fig.align='center',fig.width=4,fig.height=3,echo=FALSE}
par(mfrow=c(1,2))
...
We also noted that there was near perfect separation across subjects for some of our calculated features. For instance, the average skin temperature (TEMP) captured by E4 perfectly separates all subjects besides 4, 8, 11, 15, which can be seen in Figure 3.
...
#### II. Results 
After performing a stepwise backward selection on the model, 11 features appear to be statistically significant in discriminating between stress and amusement conditions in the model (these features are shown below in Table 3). It appears that the model has a 94.93% prediction accuracy (95% CI = (91.64, 97.20)). The confusion matrix shows that the model has the sensitivity rate of 91.40%, while a specificity rate 96.72%. 

The ROC curve is plotted to display the trade-off between sensitivity and specificity of the model. We picked the threshold of 0.5 to compare the performance of our predictions to random guesses. The model has an area under the curve (AUC) of 0.9885, which shows that our predictions are better than random guesses.
...
basicplot + style_roc(theme = theme_grey) +
  labs(title = "Chest and Wrist Model",caption = "Figure 4") + theme(plot.title = element_text(family = "Helvetica", face = "bold", size = (8))) + annotate("text", x = .75, y = .25, label = paste("AUC =", round(calc_auc(basicplot)$AUC, 4))) 
...
MultFac <- round(exp(rounded$estimate),3)
stresslikelihood <- as.factor(ifelse(MultFac > 1, 'more likely', 'less likely'))
full_coefficient <- data.frame("Terms" = c(rounded$term),
"Estimate" = c(rounded$estimate), "CI" = c("(0.401, 1.625)", "(0.053,0.835)", "(-0.080,-0.646)", "(-0.627,1.525)", "(-8.456, -5.426)","(-3.260,-2.013)","(0.643,1.996)","(-1.572,-0.330)", "(-0.097, 1.266)", "(0.270,1.253)", "(-5.999,-3.495)", "(-0.130,1.416)"),"MultFac" = c(MultFac), stresslikelihood)
colnames(full_coefficient) <- c('Terms','Estimate','95% CI',"Multiplicative Factor","Stress Likelihood")
kable(full_coefficient)
```

Table 3 displays the coefficient estimates ($\beta_i$) of each significant features ($x_{feature_i}$) as well as their 95% confidence interval.Holding all other features constant, as $x_{feature_i}$ increases by one standard deviation, the odds of stress vs amusement increase by a multiplicative factor of $e^{\beta_{i}}$, which is calculated in the fourth column. 
If this factor is smaller than one, the subject is less likely to be stressed. On the other hand, if it is greater than one, the subject is more likely to be stressed. 

Our model shows that when holding all other features at constant, the increase in one standard deviation of the rate of change in electrodermal actitivity (both from RespiBan and E4), volume of air inhale per minute (RespiBan), the average accelerometry (E4),the rate of change in body temperature (RespiBan), the average skin temperature (E4) or the average heart rate variability (E4) results in the more likelihood of being stressed. 

On the other hand, when holding all other features at constant, the increase in one standard deviation of number of skin conductivity responses (both from RespiBan and E4), the average body temperature (RespiBan), or the average heart rate variability (RespiBan) results in the less likelihood of being stressed.

One interesting finding is that an increase in the average body temperature captured by RespiBan leads to the less likelihood of being stressed, while an increase in the average skin temperature captured by E4 leads to the more likelihood of being stressed. These opposite reactions are not due to correlation because the two features have a -0.02 correlation. However, we found an increase in HRV from RespiBan results in less likelihood of being stressed, while an increase HRV from E4 leads to the more likelihood of being stressed. These may be due to the small correlation of 0.22 between the two features.

### 3.2 Wrist model 

#### I. Methodology
We wish to determine whether stress (or amusement) can be detected using only the wrist-worn wearable. For this model, we used the same training and test dataset as the previous model. We performed a logistic regression model with the 5 features measured by Empatica 4 (Table 2) on our training data. These features do not perfectly separate the data by subject, so we included subject indicators to account for subject effects. We also included all possible interactions in our model (besides interactions of features with subject indicators). 
The formula of the model is: 

$$
\begin{aligned}
log(\frac {\pi_i}{1-\pi_i}) = \beta_0 + \sum_{i=1}^{13} \beta_{i} x_{feature_i}+ \sum_{j=1}^{17} \beta_{2j} I(subjectID_i = j) + \sum_{i,k=1;i \neq k } \beta_{3i} x_{feature_i}*x_{feature_k}
\end{aligned}
$$
...
#### II. Results 
After performing the stepwise backward selection on the model, it appears that all features are statistically significant besides some subject indicators and the interaction of HRV with EDA slope as well as EDA slope with SCR number of segments. It appears that the model has a 81.16% prediction accuracy (95% CI = (76.04, 85.60)). The confusion matrix also shows that the model has the sensitivity rate of 66.67%, while a specificity rate 88.52%.

The ROC curve is plotted to display the trade-off between sensitivity and specificity of the models. We picked the threshold of 0.5 to compare the performance of our predictions to random guesses. The model has the area under the curve (AUC) of 0.8755, as shown in Figure 5. 

```{r wrist ROC, echo=FALSE, fig.align='center', message=FALSE, warning=FALSE,comment=NA, fig.width=3,fig.height=3}
true_affect <- test$affect
...
wristdata <- data.frame(affect = true_affect, 
                    interaction = wrist_int, stringsAsFactors = FALSE)

intplot <- ggplot(wristdata, aes(d = affect, m = interaction)) + geom_roc(n.cuts = 0) 
intplot + style_roc(theme = theme_grey) +
...
"Estimate" = c(rounded_wrist$estimate), "CI" = c("(1.036,2.934)", "(1.319,2.168)", "(-1.074,-0.446)", "(0.518,0.949)", "(-2.368,-1.712)","(0.870,0.061)","(-4.232,-1.412)","(-0.907,1.207)", "(-2.966,6.258)", "(-4.725,-1.666)", "(-2.858,0.063)", "(-3.942,-0.380)", "(-0.258,2.081)", "(-7.023,-4.116)", "(-3.175,-0.403)", "(-1.051,1.081)", "(-0.975,1.366)", "(-0.686,1.428)", "(-1.426,0.723)", "(-0.639,1.806)", "(0.280,0.934)", "(-1.271,-0.380)", "(0.146,0.953)", "(0.333,0.958)", "(-1.013,-0.501)", "(0.221,0.557)", "(0.321,0.872)"))

colnames(wrist_coefficient) <- c('Terms','Estimate','95% CI')
kable(wrist_coefficient)
```

Table 4 displays the coefficient estimates ($\beta_i$) of each significant features and their interaction ($x_{feature_i}$) as well as the corresponding 95% confidence interval.

#### Main effects interpretation:

* For a given subject, while all other features are at their mean, when the average magnitude of acceleration (ACC) increases by one standard deviation, the likelihood of stress increases by a multiplicative factor of
5.607 ($e^{1.724}$). 

* For a given subject, while all other features are at their mean, when the average heart rate variability (HRV) increases by one standard deviation, the likelihood of stress increases by a multiplicative factor of
0.464 ($e^{-0.767}$). 

* For a given subject, while all other features are at their mean, when the rate of change of electrodermal activity (EDA) increases by one standard deviation, the likelihood of stress increases by a multiplicative factor of 2.073 ($e^{0.729}$). 

* For a given subject, while all other features are at their mean, when number of skin conductivity responses (SCR) increases by one standard deviation, the likelihood of stress increases by a multiplicative factor of 0.132 ($e^{-2.024}$). 

* For a given subject, while all other features are at their mean, when the average skin temperature increases (TEMP) by one standard deviation, the likelihood of stress increases by a multiplicative factor of 0.629 ($e^{-0.463}$). 

Based on these results, an increase in one standard deviation of the average magnitude of acceleration (ACC) or the rate of change of electrodermal activity (EDA) while holding other features at their mean leads to the more likelihood of being stressed for a given subject. On the other hand, an increase in one standard deviation of the average heart rate variability (HRV), the number of skin conductivity responses (SCR), or the average skin temperature (TEMP) while holding other features at their mean results in the less likelihood of being stressed for a given subject.  

#### Interaction terms interpretation:
While all other features are at their mean and $x_{feature_k}$ is one standard deviation above its mean, when $x_{feature_i}$ increases by one standard deviation, the likelihood of stress increases by a multiplicative factor of $e^{\beta_i + \beta_{3i}}$. For instance:

* When all other features are at their mean and the average heart rate variability (HRV) is one standard deviation above its mean, as the rate of change of electrodermal activity (EDA) increases by one standard deviation, the likelihood of stress increases by a multiplicative factor of 3.881 ($e^{0.729 + 0.627}$).

### 3.3 Heterogeneity Across Subjects
While we were revising our model, we observed an interesting trend with the chest temperate slope feature and electrodermal activity slope feature. In the model with covariates and interactions with the covariates and subject id, of the interaction coefficients that were significant, some of the chest temperature slope interactions had different relationships with the likelihood of being stressed.  

The interaction effects of subjects 2, 3, 4,  5, 6, 11, 13 and chest temperature slope had a negative effect on the likelihood of being stressed which is opposite of the main effect of chest temperature slope on the likelihood of being stressed and opposite of the interaction effect of subject 14 and chest temperature slope of the likelihood of being stressed. For example, for a 1 standard deviation in crease in chest temperature slope, the likelihood of subject 2 being stressed decreases by a multiplicative factor of $e^{1.29}$. However, for a 1 standard deviation in crease in chest temperature slope, the likelihood of subject 14 being stressed increases by a multiplicative factor of $e^{8.53}$. This indicates that there exists heterogeneity between subjects in regards to stress vs. amusement states. 

The interaction effects of subjects 2, 3, 4, 5, 6, 7, 8, 9, 15, 17 and chest electrodermal activity slope had a positive effect on the likelihood of being stressed which is opposite of the main effect of chest electrodermal activity slope on the likelihood of being stressed and opposite of the interaction effect of subject 11 and chest electrodermal activity slope of the likelihood of being stressed. For example, for a 1 standard deviation in crease in chest electrodermal activity slope, the likelihood of subject 8 being stressed increases by a multiplicative factor of $e^{2.34}$. However, for a 1 standard deviation in crease in chest electrodermal activity slope, the likelihood of subject 15 being stressed decreases by a multiplicative factor of $e^{4.02}$. This also indicates that there exists heterogeneity between subjects in regards to stress vs. amusement states.
...
```{r wrist model for 2 min window, include=FALSE}
model_expression = affect ~ (wrist_ACC_magnitude_mean+wrist_BVP_HRV_mean+wrist_EDA_slope+wrist_SCR_num_segments+wrist_TEMP_mean)^2 - wrist_EDA_slope:wrist_SCR_num_segments + subject_id
DO_STRIDE = FALSE
...
```
We also experimented with different window shifts and window lengths, thinking that a smaller window shift and/or larger window length would lead to increased dependence between observations and a larger discrepancy between sample size and effective sample size. Examining the wrist-only model since it had the most room for improvement, we used a two-minute window (compared to the original one-minute window) and found that for a 10 second shift, the accuracy increased by 1.81%. For the two-minute window, increasing the shift to 30 seconds seemed negligible with a 0.36% decrease in accuracy. Increasing the shift also seemed negligible for the one-minute window with a 0.35% increase in accuracy.
